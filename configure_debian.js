...
async function configureDebian() {
  try {
    console.log('=== 配置Debian容器桌面环境 ===');

    console.log('\nStep 1: 停止Termux上的VNC服务');
    await executeCommand('vncserver -kill :1 2>/dev/null || true', '停止Termux VNC', true);
    await executeCommand('pkill -f novnc_proxy 2>/dev/null || true', '停止noVNC代理', true);
    ...
    console.log('\nStep 6: 创建目录');
    await executeCommand('proot-distro login debian -- bash -c "mkdir -p ~/Desktop ~/.config/autostart ~/.vnc"', '创建目录');
