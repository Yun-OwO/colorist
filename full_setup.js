...
async function fullSetup() {
  try {
    console.log('=== 完整部署Debian桌面环境 ===');

    console.log('\nStep 1: 更新Termux包管理器');
    await executeCommand("cat > $PREFIX/etc/apt/sources.list << 'EOF'\ndeb https://mirrors.tuna.tsinghua.edu.cn/termux/apt/termux-main stable main\nEOF", '配置Termux国内镜像源');
    await executeCommand('pkg update -y && pkg upgrade -y', '更新Termux');

    console.log('\nStep 2: 安装proot-distro');
    await executeCommand('pkg install proot-distro -y', '安装proot-distro');

    console.log('\nStep 3: 安装Debian容器');
    await executeCommand('proot-distro remove debian 2>/dev/null || true', '删除旧容器');
    await executeCommand('proot-distro install debian', '安装Debian');

    console.log('\nStep 4: 配置Debian国内apt源');
    ...
    console.log('\nStep 6: 安装桌面环境');
    await executeCommand('proot-distro login debian -- bash -c "apt install -y xfce4 xfce4-goodies tightvncserver novnc websockify"', '安装XFCE和VNC');
    ...
    console.log('\nStep 14: 检查服务状态');
    await executeCommand('proot-distro login debian -- bash -c "systemctl status vncserver"', '检查服务状态');

    console.log('\nStep 15: 创建Termux启动脚本');
    ...
    console.log('\nStep 17: 检查端口状态');
    await executeCommand('ss -tlnp', '检查端口监听');
